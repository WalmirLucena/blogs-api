...
Este projeto consiste em desenvolver uma API de um CRUD posts de blog (com o Sequelize). Começando pela API, que vai conter alguns endpoints (seguindo os princípios do REST) que estarão conectados ao banco de dados.

Primeiro, será criada uma tabela para os usuários que desejam se cadastrar na aplicação. Após isso, será criada também uma tabela de Categorias para os Posts e por fim a tabela de Posts, guardando todas as informações dos posts realizados na plataforma.
...
# Habilidades 
 ...
 - Utilizar no back-end `ORM` com o pacote `sequelize` do `npm`
...
Objetivo é desenvolver uma aplicação em `Node.js` usando o pacote `sequelize` para fazer um `CRUD` de posts.
